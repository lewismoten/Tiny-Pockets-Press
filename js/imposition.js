...
TPP.bestGrid = function (sheet, unit) {
  ...
  options.forEach(function (option) {
    ...
    if (!best || count > best.count)
      best = Object.assign({}, option, { cols, rows, count });
  });
  return best;
};
...
TPP.signaturePlan = function (pages, signatureSize) {
  const interior = pages.filter(function (page) {
    return !page.cover;
  });
...
TPP.signatureMark = function (
  ...
  const size = Math.max(
    0.08,
    Math.min(0.14, settings.page.w * 0.08, settings.page.h * 0.08),
  );
  mark.className = "signature-mark";
  mark.style.left = x + settings.page.w - size / 2 + "in";
  mark.style.top =
    y +
    TPP.signatureMarkY(settings, signatureIndex, signatureCount, size) +
    "in";
  mark.style.width = size + "in";
  mark.style.height = size + "in";
  mark.style.background = TPP.signatureMarkColor(
    signatureIndex,
    signatureCount,
  );
...
TPP.coverCutColor = function (settings) {
  ...
  const black = Math.min(
    TPP.contrastRatio(settings.coverBg1, "#000000"),
    TPP.contrastRatio(settings.coverBg2, "#000000"),
  );
  const white = Math.min(
    TPP.contrastRatio(settings.coverBg1, "#ffffff"),
    TPP.contrastRatio(settings.coverBg2, "#ffffff"),
  );
...
TPP.coverWrap = function (settings) {
  const wrapCap = Math.min(settings.page.w, settings.page.h) * 0.25;
  return settings.wrapCover
    ? Math.min(Math.max(0, Number(settings.wrapInside) || 0), wrapCap)
    : 0;
};
TPP.coverCornerGeometry = function (settings, w, h) {
  ...
  return {
    inset: inset,
    thickness: t,
    tip: Math.min(2 * inset - t, w / 2, h / 2),
  };
};
...
TPP.coverCutClipPath = function (settings, w, h) {
  ...
  const topRun = Math.min(
    topOuter,
    Math.max(0, w / 2 - xL),
    Math.max(0, xR - w / 2),
  );
  const sideRun = Math.min(
    leftOuter,
    Math.max(0, h / 2 - yT),
    Math.max(0, yB - h / 2),
  );
  ...
  const ix = function (value) {
    return Math.max(trim, Math.min(w - trim, value));
  };
  const iy = function (value) {
    return Math.max(trim, Math.min(h - trim, value));
  };
  const pct = function (value, total) {
    return ((value / total) * 100).toFixed(4) + "%";
  };
  const pt = function (x, y) {
    return pct(ix(x), w) + " " + pct(iy(y), h);
  };
  return (
    "polygon(" +
    [
...
TPP.sewingGuidePositions = function (span, count) {
  const usableInset = Math.min(
    Math.max(span * 0.18, 0.12),
    Math.max(0.12, span * 0.32),
  );
...
TPP.renderInterior = function () {
  ...
  document.getElementById("interiorSummary").innerHTML =
    ...
    " pages max. Interior booklet imposition has " +
    frontBlocks.length * 2 +
    " front/back side blocks. " +
    per +
    " blocks fit per sheet side. " +
    (settings.duplexBackSides
      ? "Duplex output keeps front/back pages aligned for automatic duplex printing."
      : "Non-duplex output rotates the reverse sides for manual folding and feeding.");
  const renderSidePage = function (
    label,
    blocks,
    pageIndex,
    rotate180,
    ...
    const sy =
      (settings.sheet.h - grid.rows * grid.h) / 2 + settings.printTopOffset;
    for (let i = 0; i < per; i++) {
      const block = blocks[pageIndex * per + i];
      if (!block) continue;
      const left = pages[block.pages[0] - 1] || {
        n: block.pages[0],
        type: "blank",
        html: "",
      };
      const right = pages[block.pages[1] - 1] || {
        n: block.pages[1],
        type: "blank",
        html: "",
      };
      ...
      const showSpineGuides =
        settings.showFoldGuides && TPP.signatureSpineSide(block.side);
  ...
  for (let side = 0; side < printSheets; side++) {
    ...
    renderSidePage(
      settings.duplexBackSides
        ? "Interior sheet back " + (side + 1)
        : "Interior reverse side " + (side + 1),
      backBlocks,
      side,
      !settings.duplexBackSides,
      settings.duplexBackSides,
...
TPP.renderCover = function () {
  ...
  const copies = settings.coverCopiesMax
    ? grid.count
    : Math.min(grid.count, Math.max(1, Number(settings.coverCopies) || 1));
  document.getElementById("coverSummary").innerHTML =
    "Cover stock preview. Cover footprint " +
    w.toFixed(3) +
    " × " +
    h.toFixed(3) +
    " in, including spine/wrap/material allowances. " +
    copies +
    " cop" +
    (copies === 1 ? "y" : "ies") +
    " shown.";
  ...
  const sx =
    copies === 1
      ? (settings.sheet.w - w) / 2
      : (settings.sheet.w - grid.cols * grid.w) / 2;
  const sy =
    (copies === 1
      ? (settings.sheet.h - h) / 2
      : (settings.sheet.h - grid.rows * grid.h) / 2) + settings.printTopOffset;
  const front = pages.find(function (p) {
    return p.role === "front";
  });
  const back = pages.find(function (p) {
    return p.role === "back";
  });
  for (let i = 0; i < copies; i++) {
    ...
    artwork.appendChild(
      TPP.pageEl(back, settings, wrap, wrap, false, false, {
        w: settings.page.w,
        h: settings.page.h,
      }),
    );
    ...
    artwork.appendChild(
      TPP.pageEl(
        front,
        settings,
        wrap + settings.page.w + spineW,
        wrap,
        false,
        false,
        { w: settings.page.w, h: settings.page.h },
      ),
    );
    TPP.guides(
      sheet,
      Object.assign({}, settings, { showFoldGuides: false }),
      x,
      y,
      w,
      h,
